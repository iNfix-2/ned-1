...
function ArticlePage({ article, related, onBack, onOpen, onOpenContact }) {
  return (
    <main className="flex-grow bg-black text-zinc-300 pb-20">
      ...
      <div className="max-w-[760px] mx-auto px-5 sm:px-8 pt-10">
        ...
        <p className="text-xs font-semibold tracking-wider uppercase text-zinc-400 mb-3">{article.category}</p>
        <h1 className="font-normal text-3xl sm:text-[2.6rem] leading-tight text-white">{article.title}</h1>
        ...
        <img loading="lazy" decoding="async"
          ...
          // natural aspect ratio: no cropping or zooming, so wide photos stay sharp
          className="w-full h-auto rounded-lg mt-8"
        />

        <div className="mt-8 space-y-5 text-[15px] sm:text-base leading-7 text-zinc-400">
          <p className="text-lg leading-8 text-zinc-200">{article.summary}</p>
          {(article.body || []).map((block, i) => {
            if (block.type === 'h') return <h2 key={i} className="font-normal text-xl sm:text-2xl text-white pt-6">{block.text}</h2>;
            ...
            if (block.type === 'img') return (
              <figure key={i} className="pt-4 space-y-2">
                ...
                {block.caption && <figcaption className="text-sm text-zinc-400">{block.caption}</figcaption>}
              </figure>
            );
            if (block.type === 'quote') return (
              <blockquote key={i} className="border-l-4 border-accent pl-5 py-1 my-2">
                <p className="italic text-zinc-200">“{block.text}”</p>
                {block.cite && <cite className="block mt-2 text-sm not-italic font-semibold text-accent-bright">— {block.cite}</cite>}
              </blockquote>
            );
        ...
        <div className="mt-10">
          <button
            ...
            className="px-6 py-3 rounded-full bg-accent hover:bg-blue-600 text-white text-xs font-semibold tracking-wider uppercase transition-colors"
          >
            Contact Nethawk
          </button>
        </div>
        ...
        {related.length > 0 && (
          <section className="mt-16 pt-10 border-t border-white/10">
            <h2 className="font-normal text-2xl text-white mb-6">Explore More</h2>
              ...
              {related.map((r) => (
                ...
                >
                  ...
                  <div className="py-3 pr-4 min-w-0">
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">{r.category}</p>
                    <p className="text-sm sm:text-base font-semibold text-white leading-snug mt-1 line-clamp-2">{r.title}</p>
                    <p className="text-xs text-zinc-400 mt-1">{r.date}</p>
                  </div>
                </button>
              ))}
            </div>
    ...
    <div className="min-h-screen bg-[#000000] text-white flex flex-col font-sans selection:bg-accent selection:text-white antialiased">
      ...
      <main className="flex-grow pt-28 pb-20 space-y-16 sm:space-y-24">
        ...
        <section className="px-6 sm:px-12 max-w-[1920px] mx-auto text-left space-y-6">
          <h1 className="font-normal text-4xl sm:text-6xl lg:text-7xl tracking-tight text-white">
            In the Media
          </h1>
          <p className="text-zinc-300 text-sm sm:text-base lg:text-lg max-w-3xl font-light leading-relaxed">
            ...
            >
              ...
              <div className="p-8 sm:p-10 space-y-6 flex-grow flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <h2 className="font-normal text-xl sm:text-2xl text-white group-hover:text-accent-bright transition-colors leading-snug">
                    {article.title}
                  </h2>
                ...
                <div className="pt-6 border-t border-white/10 flex items-center justify-between">
                  <span className="text-xs text-accent-bright font-medium">Read Full Statement</span>
