    ...
    <div className="min-h-screen bg-[#000000] text-white flex flex-col font-sans selection:bg-accent selection:text-white antialiased">
      ...
      <main className="flex-grow pt-28 pb-20 space-y-24 sm:space-y-32">
        ...
        <section className="px-6 sm:px-12 max-w-[1920px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* GAMALINK Card */}
          <div className="rounded-[36px] bg-[#0a0a0a] border border-white/15 p-8 sm:p-12 space-y-8 flex flex-col justify-between shadow-2xl">
            <div className="space-y-4">
              <h2 className="font-normal text-2xl sm:text-3xl text-white">{gamalink.title}</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                {gamalink.specs.slice(0, 6).map((sp, idx) => (
                  <div key={idx} className="p-3 rounded-[16px] bg-white/5 border border-white/5 space-y-1">
                    <span className="text-zinc-400 block text-[10.5px]">{sp.label}</span>
          ...
          <div className="rounded-[36px] bg-[#0a0a0a] border border-white/15 p-8 sm:p-12 space-y-8 flex flex-col justify-between shadow-2xl">
            <div className="space-y-4">
              <h2 className="font-normal text-2xl sm:text-3xl text-white">{gamasar.title}</h2>
              <p className="text-zinc-300 text-sm sm:text-base font-light leading-relaxed">
                {gamasar.desc}
              </p>
            </div>
        ...
        <section className="px-6 sm:px-12 max-w-[1920px] mx-auto space-y-8">
          <div className="space-y-2">
            <h2 className="font-normal text-2xl sm:text-4xl text-white">Active Spaceflight Missions</h2>
          </div>
            ...
            {SPACE_PRODUCTS.missions.map((m, idx) => (
              ...
              >
                <div className="space-y-3">
                  ...
                  <p className="text-zinc-400 text-xs sm:text-sm font-light leading-relaxed">{m.desc}</p>
                </div>
                <div className="pt-4 border-t border-white/10">
                  <a
                    ...
                    className="text-xs text-accent-bright hover:text-white flex items-center space-x-1.5 transition-colors"
                  >
